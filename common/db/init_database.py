...
class DatabaseInitializer:
    ...
    TABLES_DDL = {
        ...
        # 卡密库存/销售明细表（逐条记录批量数据卡券卖出的卡密）
        "xy_card_stock": """
            CREATE TABLE IF NOT EXISTS xy_card_stock (
                id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '库存记录ID',
                card_id BIGINT NOT NULL COMMENT '所属卡券ID',
                user_id BIGINT DEFAULT NULL COMMENT '所属用户ID',
                card_no VARCHAR(255) DEFAULT NULL COMMENT '卡号/账号',
                card_secret VARCHAR(255) DEFAULT NULL COMMENT '卡密/密码',
                content VARCHAR(1024) DEFAULT NULL COMMENT '原始整行内容',
                status VARCHAR(16) NOT NULL DEFAULT 'sold' COMMENT '状态：unsold-未售 sold-已售',
                order_id VARCHAR(64) DEFAULT NULL COMMENT '关联订单ID',
                account_id VARCHAR(80) DEFAULT NULL COMMENT '关联账号(卖家cookie_id)',
                buyer_id VARCHAR(80) DEFAULT NULL COMMENT '买家ID',
                cost_price VARCHAR(32) DEFAULT NULL COMMENT '成本单价',
                channel VARCHAR(64) DEFAULT NULL COMMENT '发货渠道',
            ...
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='卡密库存/销售明细表';
        """,
    ...
    COLUMN_MIGRATIONS = {
        "xy_message_notifications": [
            ("chat_notify_enabled", "TINYINT(1) NOT NULL DEFAULT 1 COMMENT '聊天消息通知开关（方案A）：0=关闭买家聊天通知，不影响订单/发货通知'", "enabled"),
        ],
        "xy_listing_monitor_tasks": [
